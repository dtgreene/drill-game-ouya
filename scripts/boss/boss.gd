...
func _ready():
	...
	connect("area_exit", self, "area_exit")
	
	sprite.connect("finished", self, "animation_finished")
	player.connect("used_explosive", self, "player_used_explosive")
	
	set_pos(Vector2(15 * 32, y_base + 128))
...
func _process(delta):
	...
	if state == States.EMERGE:
		...
		if position.y > y_base:
			set_pos(Vector2(position.x, position.y - delta * 32))
		else:
			set_pos(Vector2(position.x, y_base))
		...
		set_pos(Vector2(position.x + delta * 96 * walk_sign, position.y))
		...
		if (current_frame == 3 || current_frame == 11) && current_frame != last_step_frame:
			...
			var voice = effects_player.play("boss_stomp")
			effects_player.set_pitch_scale(voice, 1 + rand_range(0, 0.4) - 0.2)
...
func change_state(value):
	...
	if value == States.EMERGE:
		collision_shape.set_pos(Vector2(0, 4))
...
func animation_finished():
	if state == States.ATTACK_BLAST:
		...
		spawner.set_pos(Vector2(position.x + x_offset, position.y))
		...
		slam.set_pos(Vector2(position.x, position.y + 44))
