	...
	if terrain_step == 0:
		...
		for col in range(col_count):
			for row in range(row_count):
				...
				used_instance.set_pos(Vector2(col * 32, row * 32))
...
func generate_terrain_process():
	if terrain_step == 0:
		...
		for i in range(void_shape_count):
			var col = floor(rand_range(0, 1) * col_count)
...
func update_chunks(position):
	...
	if col != last_chunk_col || row != last_chunk_row:
		...
		for chunk_index in chunks_new:
			chunks[chunk_index].set_hidden(false)
		...
		for chunk_index in chunks_active:
			...
			if !chunks_new.has(chunk_index):
				# deactivate chunk
				chunks[chunk_index].set_hidden(true)
...
func explode(pos, half_width_height, unfill, gas = false): 
	...
	while col <= stop_col:
		while row <= stop_row:
			...
			used_instance.set_pos(Vector2(col * 32 + rand_range(0, 8) - 4, row * 32 + rand_range(0, 8) - 4))
