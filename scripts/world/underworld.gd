...
func _ready():
	...
	# add ground edges
	var y = (GlobalValues.row_count + 5) * 32
	for col in range(GlobalValues.col_count):
		used_instance = BlockScene.instance()
		used_instance.set_pos(Vector2(col * 32, y))
		used_instance.un_fill()
