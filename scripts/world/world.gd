...
func _ready():
	...
	# enable loop on some sounds
	var sounds = [
		load("res://assets/sounds/engine_idle.wav"),
		load("res://assets/sounds/engine_thrust.wav"),
		load("res://assets/sounds/drill.wav")
	]
	
	for sound in sounds:
		sound.set_loop_format(1)
		sound.set_loop_end(sound.get_length())
...
func terrain_ready():
	...
	player.set_pos(Vector2(8 * 32, -32))
	add_child(player)
...
func ui_screen_toggle(open):
	if open:
		building_music.play()
		main_music_pos = main_music.get_pos()
		main_music.stop()
	else: 
		building_music.stop()
...
func player_depth_change(depth):
	...
	if !overworld_hidden: 
		if depth > 0:
			...
			overworld.set_hidden(true)
	else:
		if depth <= 100:
			overworld_hidden = false
			overworld.set_hidden(false)
	
	if !underworld_hidden:
		if depth < 7000:
			underworld_hidden = true
			underworld.set_hidden(true)
	else:
		if depth >= 7000:
			underworld_hidden = false
			underworld.set_hidden(false)
